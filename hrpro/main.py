...
def main(args):
    ...
    if args.stage == 1:
        ...
        train_loader = data.DataLoader(
            dataset(args, phase="train", sample="random", stage=args.stage),
            batch_size=1,
            shuffle=True,
            num_workers=args.num_workers,
        )
        test_loader = data.DataLoader(
            dataset(args, phase="test", sample="random", stage=args.stage),
            batch_size=1,
            shuffle=False,
            num_workers=args.num_workers,
        )
        ...
        if args.mode == "train":
            ...
            log_filepath = os.path.join(
                args.log_path_s1, "{}.score".format(args.dataset)
            )
            ...
            for step in process_bar:
                ...
                if step % args.test_iter == 0:
                    ...
                    test_info["elapsed"].append(
                        str(datetime.timedelta(seconds=time.time() - start_time))
                    )
                    test_info["now"].append(
                    ...
                    if test_mAP > best_mAP:
                        ...
                        torch.save(
                            ...
                            os.path.join(
                                args.model_path_s1,
                                "model1_seed_{}.pkl".format(args.seed),
                            ),
                        )

                    print(
                        "\n Current test_mAP:{:.4f} best_mAP:{:.4f}".format(
                            test_mAP, best_mAP
                        )
                    )
            ...
            model.load_state_dict(
                torch.load(
                    os.path.join(
                        args.model_path_s1, "model1_seed_{}.pkl".format(args.seed)
                    )
                )
            )
    ...
    # --------------------------------------------------Instance-level Optimization-------------------------------------------------------#
        ...
        if args.mode == "train":
            ...
            log_filepath = os.path.join(
                args.log_path_s2, "{}.score".format(args.dataset)
            )
            initial_log(log_filepath, args)

            optimizer = torch.optim.Adam(
                model.parameters(), lr=args.lr2, weight_decay=args.weight_decay
            )
            ...
            for epoch in process_bar:
                ...
                if epoch % args.test_epoch == 0:
                    test_mAP = I_test(
                        epoch, args, test_dataset, model, logger, test_info
                    )
                    test_info["loss"].append(loss)
                    test_info["elapsed"].append(
                        str(datetime.timedelta(seconds=time.time() - start_time))
                    ...
                    if test_mAP > best_mAP:
                        ...
                        torch.save(
                            ...
                            os.path.join(
                                args.model_path_s2,
                                "model2_seed_{}.pkl".format(args.seed),
                            ),
                        )
            ...
            model.load_state_dict(
                torch.load(
                    os.path.join(
                        args.model_path_s2, "model2_seed_{}.pkl".format(args.seed)
                    )
                )
            )
