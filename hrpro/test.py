...
def S_test(net, args, test_loader, logger, step, test_info, subset="test"):
    ...
    for sample in tqdm(test_loader):
        _data, _vid_label, _vid_name, _vid_len, _vid_duration = (
            sample["data"],
            sample["vid_label"],
            sample["vid_name"],
            sample["vid_len"],
        ...
        outputs = net(_data.to(args.device).float())
        ...
        for b in range(_data.shape[0]):
            ...
            for i in range(len(args.act_thresh_cas)):
                ...
                for c in range(len(pred)):
                    ...
                    seg_list.append(pos)
                proposals = utils.get_proposal_oic(
                    args,
                    seg_list,
                    cas_temp,
                    score_np,
            ...
            for i in range(len(args.act_thresh_agnostic)):
                ...
                zero_location = np.where(
                    agnostic_score_temp[:, :, 0] < args.act_thresh_agnostic[i]
                )
            ...
            if args.mode == "train" or args.mode == "infer":
                final_proposals = utils.post_process(
                    args, vid_name, proposal_dict, test_loader
                )
            else:
                final_proposals = []
    ...
    if args.mode == "train" or args.mode == "infer":
        ...
        test_map = log_evaluate(
            args, step, test_acc, logger, json_path, test_info, subset
        )
        return test_map
...
def I_test(epoch, args, test_dataset, net, logger, test_info):
    ...
    for sample in tqdm(test_loader):
        ...
        proposals_input = [
            torch.from_numpy(prop).float().to(args.device) for prop in proposals
        ]
