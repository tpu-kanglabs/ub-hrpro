...
def extract_region_feat(seq, embeded_feature):
    ...
    for i in range(len(range_idx) // 2):
        if range_idx[2 * i + 1] - range_idx[2 * i] < 1:
            continue
        feature_lsts.append(
            embeded_feature[range_idx[2 * i] + 1 : range_idx[2 * i + 1] + 1].clone()
        )
...
def get_proposal_oic(
    args, tList, wtcam, vid_score, c_pred, v_len, num_segments, v_duration
):
    ...
    for i in range(len(tList)):
        ...
        if temp_list.any():
            ...
            for j in range(len(grouped_temp_list)):
                ...
                outer_s = max(
                    0, int(grouped_temp_list[j][0] - args._lambda * len_proposal)
                )
                outer_e = min(
                    int(wtcam.shape[0] - 1),
                    int(grouped_temp_list[j][-1] + args._lambda * len_proposal),
                )
                outer_temp_list = list(
                    range(outer_s, int(grouped_temp_list[j][0]))
...
def get_prediction(proposals, data_dict, dataset):
    ...
    for i in range(proposals.shape[0]):
        ...
        proposal_dict[c].append(
            [proposals[i, 0] / t_factor, proposals[i, 1] / t_factor, c, c_score]
        )
