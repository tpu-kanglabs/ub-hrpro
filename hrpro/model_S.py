...
class Reliable_Memory(nn.Module):
    def __init__(self, num_class, feat_dim):
        ...
        self.proto_num = 1
        self.proto_vectors = torch.nn.Parameter(
            torch.zeros([self.num_class, self.proto_num, self.feat_dim]),
            requires_grad=False,
        )
    ...
    def init(self, args, net, train_loader):
        ...
        with torch.no_grad():
            ...
            temp_loader = data.DataLoader(
                train_loader.dataset, batch_size=1, shuffle=False, num_workers=4
            )
            for sample in temp_loader:
                _data, vid_label, point_anno = (
                    sample["data"],
                    sample["vid_label"],
                    sample["point_label"],
                )
                outputs = net(
                    _data.to(args.device).float(), vid_label.to(args.device).float()
                )
    ...
    def update(self, args, feats, act_seq, vid_label):
        ...
        for c in feat_list.keys():
            if len(feat_list[c]) > 0:
                ...
                self.proto_vectors[c] = (1 - self.proto_momentum) * self.proto_vectors[
                    c
                ] + self.proto_momentum * feat_update


class Reliabilty_Aware_Block(nn.Module):
    def __init__(
        self, input_dim, dropout, num_heads=8, dim_feedforward=128, pos_embed=False
    ):
        super(Reliabilty_Aware_Block, self).__init__()
        self.conv_query = nn.Conv1d(
            input_dim, input_dim, kernel_size=1, stride=1, padding=0
        )
        ...
        self.self_atten = nn.MultiheadAttention(
            input_dim, num_heads=num_heads, dropout=0.1
        )
...
class S_Model(nn.Module):
    ...
    def forward(self, input_features, vid_labels=None):
        ...
        embeded_feature = self.encoder(
            input_features, self.memory.proto_vectors
        )  # [B,F,T]
    ...
    def criterion(self, args, outputs, vid_label, point_label):
        vid_score, embeded_feature, cas_fuse = (
            outputs["vid_score"],
            outputs["embeded_feature"],
            outputs["cas_fuse"],
        )
        point_label = torch.cat(
            (
                point_label,
                torch.zeros((point_label.shape[0], point_label.shape[1], 1)).to(
                    args.device
                ),
            ),
            dim=2,
        )
        act_seed, bkg_seed = utils.select_seed(
            cas_fuse[:, :, -1].detach().cpu(), point_label.detach().cpu()
        )

        loss_dict = {}
        # >> base loss
        loss_vid, loss_frame, loss_frame_bkg = self.base_loss_func(
            args, act_seed, bkg_seed, vid_score, vid_label, cas_fuse, point_label
        )
        ...
        # >> feat loss
        loss_contrastive = self.feat_loss_func(
            args, embeded_feature, act_seed, bkg_seed, vid_label
        )
        ...
        loss_total = (
            ...
            + self.lambdas[1] * loss_frame
            + self.lambdas[2] * loss_frame_bkg
            + self.lambdas[3] * loss_contrastive
        )
    ...
    def base_loss_func(
        ...
        focal_weight_act = (1 - cas_sigmoid_fuse) * point_anno + cas_sigmoid_fuse * (
            1 - point_anno
        )
        ...
        loss_frame = (
            (
                (
                    focal_weight_act
                    * self.bce_criterion(cas_sigmoid_fuse, point_anno)
                    * weighting_seq_act
                ).sum(dim=2)
            ).sum(dim=1)
            / (num_actions + 1e-6)
        ...
        focal_weight_bkg = (
            1 - cas_sigmoid_fuse
        ) * point_anno_bkg + cas_sigmoid_fuse * (1 - point_anno_bkg)
        focal_weight_bkg = focal_weight_bkg**2
        loss_frame_bkg = (
            (
                (
                    focal_weight_bkg
                    * self.bce_criterion(cas_sigmoid_fuse, point_anno_bkg)
                    * weighting_seq_bkg
                ).sum(dim=2)
            ).sum(dim=1)
            / (num_bkg + 1e-6)
    ...
    def feat_loss_func(self, args, embeded_feature, act_seed, bkg_seed, vid_label):
        ...
        for b in range(act_seed.shape[0]):
            ...
            for c in gt_class:
                act_feat_lst.append(
                    utils.extract_region_feat(
                        act_seed[b, :, c], embeded_feature[b, :, :]
                    )
                )
            bkg_feat = utils.extract_region_feat(
                bkg_seed[b].squeeze(-1), embeded_feature[b, :, :]
            )
            ...
            b_sim_matrix = (
                torch.matmul(
                    bkg_feat.unsqueeze(0).expand(args.num_class, -1, -1),
                    torch.transpose(proto_vectors, 1, 2),
                )
                / 0.1
            )  # [C,t_b,N]
            b_sim_matrix = (
                torch.exp(b_sim_matrix).reshape(b_sim_matrix.shape[0], -1).mean(dim=-1)
            )  # [C]
            for idx, act_feat in enumerate(act_feat_lst):
                if act_feat is not None:
                    ...
                    a_sim_matrix = (
                        torch.matmul(
                            act_feat.unsqueeze(0).expand(args.num_class, -1, -1),
                            torch.transpose(proto_vectors, 1, 2),
                        )
                        / 0.1
                    )  # [C,t_a,N]
                    a_sim_matrix = (
                        torch.exp(a_sim_matrix)
                        .reshape(a_sim_matrix.shape[0], -1)
                        .mean(dim=-1)
                    )  # [C]
                    ...
                    loss_contra_bkg = -torch.log(
                        a_sim_matrix[c] / (a_sim_matrix[c] + b_sim_matrix[c])
                    )
