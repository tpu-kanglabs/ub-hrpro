...
class Boundary_Completeness_Regressor(torch.nn.Module):
    ...
    def forward(
        ...
        if is_training:
            ...
            for b in range(len(proposals)):
                proposal_refined = self.refine_proposals_se(
                    proposals[b], mask[b], start_reg[b], end_reg[b]
                )
                reg_loss, iou_gt = self.regression_loss(
                    proposal_refined, proposals[b], pseudo_labels[b], iou_thresh
                )
    ...
    def segments_iou(self, segments1, segments2):
        ...
        union = (
            (segments1[..., 1] - segments1[..., 0])
            + (segments2[..., 1] - segments2[..., 0])
            - intersection
        )
    ...
    def regression_loss(self, proposal_refined, proposals, pseudo_label, iou_thresh):
        ...
        reg_loss = F.smooth_l1_loss(
            refined_iou,
            torch.ones(refined_iou.shape[0]).to(refined_iou.device),
            reduction="none",
        )
...
class I_Model(torch.nn.Module):
    def __init__(self, args):
        ...
        self.roi_size = 12
        self.bcr = Boundary_Completeness_Regressor(
            self.feat_dim, dropout_ratio, self.roi_size
        )

    def forward(self, features, proposals, pseudo_labels=None, is_training=True):
        prop_features, prop_mask = self.extract_roi_features(
            features, proposals, is_training
        )
        prop_refined, iou_pred_orig, reg_loss, iou_loss = self.bcr(
            prop_features, proposals, prop_mask, pseudo_labels, 0.3, is_training
        )
        prop_refined_feature, prop_mask1 = self.extract_roi_features(
            features, prop_refined, is_training
        )
        _, iou_pred_refined, _, _ = self.bcr(
            prop_refined_feature,
            prop_refined,
            prop_mask1,
            pseudo_labels,
            0.3,
    ...
    def extract_roi_features(self, features, proposals, is_training):
        ...
        prop_features = torch.zeros((batch, max_num, self.roi_size, self.feat_dim)).to(
            features[0].device
        )
        ...
        for i in range(batch):
            ...
            fill_len = (
                torch.ceil(0.25 * len_prop.max()).long() + 1
            )  # +1 because of offset 0.5
            ...
            feat_roi = torchvision.ops.roi_align(
                feature, [boxes], [self.roi_size, 1]
            )  # [M, D, roi_size, 1]
            feat_roi = feat_roi.squeeze(3).transpose(1, 2)  # [M, roi_size, D]
            prop_features[i, : proposal.shape[0], :, :] = (
                feat_roi  # [B, M, roi_size, D]
            )
