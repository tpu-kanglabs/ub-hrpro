...
class dataset(Dataset):
    ...
    def _prepare_data(self):
        ...
        self.data_list = [
            item.strip()
            for item in list(
                open(os.path.join(self.data_path, "split_{}.txt".format(self.phase)))
            )
        ]
        ...
        # >> point label
        self.point_anno = pd.read_csv(
            os.path.join(self.data_path, "point_labels", "point_gaussian.csv")
        )

        if self.stage == 2:
            with open(
                os.path.join(
                    self.args.output_path_s1, "proposals.json"
                ),
                "r",
            ) as f:
    ...
    def __getitem__(self, idx):
        ...
        vid_label, point_label, vid_duration = self.process_label(
            vid_name, vid_len, sample_idx
        )
        if self.stage == 1:
            sample = dict(
                data=data,
            ...
            if self.phase == "test":
                ...
                if proposals.shape[0] > self.args.max_proposal:
                    proposals = proposals[
                        random.sample(
                            list(range(proposals.shape[0])), self.args.max_proposal
                        ),
                        :,
                    ]
                return dict(
            ...
            for i in range(self.num_segments):
                if i < self.num_segments - 1:
                    if int(sample_idx[i]) != int(sample_idx[i + 1]):
                        sample_idx[i] = np.random.choice(
                            range(int(sample_idx[i]), int(sample_idx[i + 1]) + 1)
                        )
                    else:
                    ...
                    if int(sample_idx[i]) < vid_len - 1:
                        sample_idx[i] = np.random.choice(
                            range(int(sample_idx[i]), vid_len)
                        )
                    else:
                        sample_idx[i] = int(sample_idx[i])
    ...
    def process_label(self, vid_name, vid_len, sample_idx):
        ...
        vid_duration, vid_fps = (
            self.gt_dict[vid_name]["duration"],
            self.gt_dict[vid_name]["fps"],
        )

        if self.num_segments == -1:
            self.t_factor_point = self.args.frames_per_sec / (vid_fps * 16)
            temp_anno = np.zeros([vid_len, self.num_class], dtype=np.float32)
            temp_df = self.point_anno[self.point_anno["video_id"] == vid_name][
                ["point", "class"]
            ]
        ...
        for vid_name in self.data_list:
            ...
            for label in PP.keys():
                for prop in PP[label]:
                    prop_new = [
                        round(prop[0] * t_factor, 3),
                        round(prop[1] * t_factor, 3),
                        prop[2],
                        self.class_idx_dict[label],
                    ]
