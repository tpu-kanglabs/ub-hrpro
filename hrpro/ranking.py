...
from utils import grouping, select_seed
...
def reliability_ranking(args, train_loader, test_loader):
    ...
    for subset in ["test", "train"]:
        snippet_result_path = os.path.join(
            args.output_path_s1, "snippet_result_{}.json".format(subset)
        )
        ...
        for sample in tqdm(data_loader):
            ...
            if subset == "train":
                ...
                # >> Reliable Proposals
                points_df = point_anno[point_anno["video_id"] == vid_name][
                    ["point", "class"]
                ].sort_values(by="point", ascending=True)
                points = []
                ...
                # >> Negative Proposals
                bkg_score = np.asarray(
                    snippet_result["bkg_score"][vid_name], dtype=np.float32
                )
                _, bkg_seed = select_seed(
                    torch.from_numpy(bkg_score).unsqueeze(0), point_label
                )
