...
def I_train(epoch, args, train_dataset, net, optimizer, logger):
    ...
    for sample in train_loader:
        features, proposals, pseudo_labels = (
            sample["data"],
            sample["proposals"],
            sample["psuedo_label"],
        )
        ...
        proposals = [
            torch.from_numpy(prop[:, :2]).float().to(args.device) for prop in proposals
        ]
        pseudo_labels = [
            torch.from_numpy(pseudo_label).float().to(args.device)
