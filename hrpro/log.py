...
def save_best_record(test_info, log_filepath):
    with open(log_filepath, "a") as f:
        f.write(
            "| {:^6s} | {:^8s} | {:^6s} | {:^6s} | {:^6s} | {:^6s} | {:^6s} | {:^6s} | {:^6s} | {:^6s} | {:^15} | {:^15} | {:^15} | {:^15} | {:^20} | \n".format(
                str(test_info["step"][-1]),
                "{:.2f}".format(100 * test_info["test_acc"][-1]),
                "{:.3f}".format(test_info["loss"][-1]),
                "{:.2f}".format(100 * test_info["mAP@0.1"][-1]),
                "{:.2f}".format(100 * test_info["mAP@0.2"][-1]),
                "{:.2f}".format(100 * test_info["mAP@0.3"][-1]),
                "{:.2f}".format(100 * test_info["mAP@0.4"][-1]),
                "{:.2f}".format(100 * test_info["mAP@0.5"][-1]),
                "{:.2f}".format(100 * test_info["mAP@0.6"][-1]),
...
def log_evaluate(args, step, test_acc, logger, json_path, test_info, subset="test"):
    ...
    anet_detection = ANETdetection(
        ...
        json_path,
        subset=subset_name,
        tiou_thresholds=args.tIoU_thresh,
        verbose=False,
        check_status=False,
        blocked_videos=args.blocked_videos,
    )
    ...
    if logger is not None:
        logger.log_value(
            "{}/average mAP[0.1:0.7]".format(log_folder), mAP[:7].mean(), step
        )
        logger.log_value(
            "{}/average mAP[0.1:0.5]".format(log_folder), mAP[:5].mean(), step
        )
        logger.log_value(
            "{}/average mAP[0.3:0.7]".format(log_folder), mAP[2:7].mean(), step
        )
        for i in range(len(args.tIoU_thresh)):
            logger.log_value(
                "{}/mAP@{:.1f}".format(log_folder, args.tIoU_thresh[i]), mAP[i], step
            )
