...
def parse_args():
    ...
    parser.add_argument(
        "--cfg", type=str, default="thumos", help="hyperparameters path"
    )
    parser.add_argument(
        "--mode", type=str, default="train", choices=["train", "test", "infer"]
    )
    parser.add_argument(
        "--stage", type=int, default=1, help="traning stage", choices=[1, 2]
    )
    parser.add_argument(
        "--seed", type=int, default=0, help="random seed (-1 for no manual seed)"
    )
    parser.add_argument(
        "--ckpt_path",
        type=str,
        default="./ckpt",
        help="root folder for saving models,ouputs,logs",
    )
