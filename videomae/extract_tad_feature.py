...
import numpy as np
import torch
from dataset import get_video_loader
...
def get_args():
    ...
    parser.add_argument(
        "--data_set",
        default="ub-moji",
        choices=["THUMOS14", "FINEACTION", "ub-moji"],
        type=str,
        help="dataset",
    )
    ...
    parser.add_argument(
        "--save_path", default="features", type=str, help="path for saving features"
    )
    ...
    parser.add_argument(
        "--ckpt_path", required=True, type=str, help="load from checkpoint"
    )
